...
class _ListaEventosV2State extends State<ListaEventosV2> {
  ...
  void dispose() {
    if (_enteredTime != null) {
      AnalyticsService.logTime('time_lista_v2', DateTime.now().difference(_enteredTime!));
    }
    super.dispose();
  }
  ...
  Future<void> _editarEvento(Evento evento) async {
    ...
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => FormularioEvento(evento: evento, source: 'v2')),
    );
    if (result == true) {
      _loadEventos();
      setState(() {});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Lista de Eventos (V2)")),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(evento.nome, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                            const SizedBox(height: 6),
                            Text(evento.descricao, maxLines: 2, overflow: TextOverflow.ellipsis),
                            const SizedBox(height: 8),
                            Text("${evento.tipo} • ${evento.data.day}/${evento.data.month}/${evento.data.year}", style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                          ],
                        ),
                      ),
                      Column(
                        children: [
                          IconButton(icon: const Icon(Icons.edit), onPressed: () => _editarEvento(evento)),
                          IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteEvento(evento.id!)),
                        ],
                      )
                    ],
                  ),
